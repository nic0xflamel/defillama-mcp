import { OpenAPIToMCPConverter, NewToolMethod as ActualNewToolMethod } from '../parser'
import { OpenAPIV3 } from 'openapi-types'
import { describe, expect, it } from 'vitest'
import { JSONSchema7 as IJsonSchema } from 'json-schema'

// Use the imported NewToolMethod or a compatible local version
interface TestToolMethod extends ActualNewToolMethod {}

// Helper function to verify tool method structure
function verifyToolMethod(actual: TestToolMethod, expected: any, toolName?: string) { // toolName might be redundant now
  expect(actual.name).toBe(expected.name)
  expect(actual.description).toBe(expected.description)
  expect(actual.inputSchema, `inputSchema ${actual.name}`).toEqual(expected.inputSchema)
  if (expected.returnSchema) {
    expect(actual.returnSchema, `returnSchema ${actual.name}`).toEqual(expected.returnSchema)
  }
}

// Helper function to verify tools structure (now a flat array)
function verifyTools(actual: TestToolMethod[], expected: TestToolMethod[]) {
  expect(actual.length).toBe(expected.length);
  actual.forEach((method: TestToolMethod, index: number) => {
    verifyToolMethod(method, expected[index]);
  });
}
...
function getParamsFromSchema(method: { inputSchema: IJsonSchema & { type: 'object' } }) { // Ensure inputSchema matches NewToolMethod
...
describe('OpenAPIToMCPConverter', () => {
  describe('Simple API Conversion', () => {
    ...
    it('converts simple OpenAPI paths to MCP tools', () => {
      ...
      expect(Object.keys(openApiLookup)).toHaveLength(1) // Key is now direct tool name

      const getPetMethod = tools.find((m: TestToolMethod) => m.name === 'getPet')
    ...
    it('truncates tool names exceeding 64 characters', () => {
      ...
      // Name is now generated by ensureUniqueName which handles truncation and suffixing
      const longNameMethod = tools.find((m: TestToolMethod) => m.name.startsWith('a'.repeat(64 - 5 - 1))) // a * 59, plus a dash, plus 4 digits for suffix
  ...
  describe('Complex API Conversion', () => {
    ...
    it('converts operations with query parameters', () => {
      ...
      const listPetsMethod = tools.find((m: TestToolMethod) => m.name === 'listPets')
    ...
    it('converts operations with request bodies using $ref', () => {
      ...
      const createPetMethod = tools.find((m: TestToolMethod) => m.name === 'createPet')
    ...
    it('converts all operations correctly respecting $ref usage', () => {
      ...
      const methodNames = tools.map((m: TestToolMethod) => m.name)
      expect(methodNames).toEqual(expect.arrayContaining(['listPets', 'createPet', 'getPet', 'updatePet']))

      tools.forEach((method: TestToolMethod) => {
  ...
  describe('Complex Schema Conversion', () => {
    ...
    it('handles deeply nested object references', () => {
      ...
      const getOrgMethod = tools.find((m: TestToolMethod) => m.name === 'getOrganization')
    ...
    it('handles recursive array references without requiring expansion', () => {
      ...
      const updateDeptMethod = tools.find((m: TestToolMethod) => m.name === 'updateDepartment')
    ...
    it('handles complex nested object hierarchies without expansion', () => {
      ...
      const getDeptMethod = tools.find((m: TestToolMethod) => m.name === 'getDepartment')
